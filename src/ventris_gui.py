...
def load_mappings():
    if not os.path.exists(MAPPINGS_FILE):
        ...
        print("Please run search.py and then config_gui.py to create and configure mappings.")
        return None
    try:
        with open(MAPPINGS_FILE, 'r') as f:
            mappings = json.load(f)
            # Filter out entries with no assigned shortcut
            return {k: v for k, v in mappings.items() if v is not None}
    except (json.JSONDecodeError, IOError) as e:
        print(f"--- ERROR: Could not read or parse mappings file: {e} ---")
        return None
...
if MAPPINGS is None:
    exit()
if not MAPPINGS:
    print("--- WARNING: No mappings are configured. The application will not launch anything. ---")
    print("--- Please run config_gui.py to set up mappings. ---")
